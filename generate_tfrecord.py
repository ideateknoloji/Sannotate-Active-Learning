...
def create_tfrecord_file():
    """
    Generate tfrecord file from csv file and images folder
    
    TO-DO : This functions further can be updated with very easy logic to generate tfrecord file for multiple folders
    """

    label_map_path = "labelmap_v7.pbtxt"
    csv_paths = ["train_media/train.csv", "test_media/test.csv"]
    image_paths = ["train_media", "test_media"]
    tfrecords = ["train.record", "test.record"]

    for csv_path, image_path, tfrecord in zip(csv_paths, image_paths, tfrecords):
        print("images path : ", image_path)
        print("csv path : ", csv_path)
        print("path to output tfrecords : ", tfrecord)
        ...
        writer.close()
        print('Successfully created the TFRecords: {}'.format(tfrecord))
